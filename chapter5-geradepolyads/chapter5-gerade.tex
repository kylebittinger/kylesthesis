...
intensity of an eigenstate is relative to its fractional $S_1$
character, the magnitude and direction of the center of gravity offset
is found by combining Equations \ref{eq:ave-s1-char} and
\ref{eq:cog-def}:
\begin{equation}
  \begin{split}
    E_{\text{ave}} 
...
By definition, we are guaranteed that the lifetime of the nominal
$S_1$ eigenstate is shorter than that of any nominal $T_{1,2}$
eigenstate.  The difference in lifetimes gives rise to changes in the
relative intensity of the $S_1$ eigenstate relative to that of the
nominal $T_{1,2}$ eigenstates as a function of time delay after
excitation of the molecules.  Changes in the relative intensity lead
to changes in the overall center of gravity, computed from the total
fluorescence intensity within a delayed time window $t$ to $t+dt$
after excitation.  In the remainder of this section, we examine the
dependence of the intensity-weighted center-of-gravity on time delay
for an incoherent ensemble of mixed $S_1 \sim T_3$ eigenstates.
...
of gravity is also decreased, due to the greater relative intensity of
$\ket{2}$ at $t=0$.
...
% The change in the center of gravity with time delay is a function of
% the amount of contrast between the relative intensities of the nominal
% $S_1$ mixed state and the remaining nominal $T_{1,2}$ mixed states.
% The relative intensity contrast, in turn, is determined by the
% difference in lifetimes between the most strongly mixed components in
% an ensemble.  For example, when the $S_1 \sim T_3$ matrix element is
% small, the local manifold of $T_{1,2}$ levels may mix only weakly with
% the $S_1$ level.  The quantity $(1-a^2)$ remains close to 1, and the
% nominal $S_1$ mixed state has a lifetime close to $\tau_s$.  The
% remaining fractional $S_1$ character, $a^2$, is distributed among all
% nominal $T_{1,2}$ mixed states.  As a result, the lifetimes of the
% nominal $T_{1,2}$ mixed states are much longer than that of the
% nominal singlet state.  The large difference in lifetimes creates a
% sharp contrast in relative intensity, when the intensity of the
% nominal $S_1$

% In cases where a $T_{1,2}$
% level may mix efficiently with an $S_1$ level, the resulting
% singlet$\sim$triplet mixed states have similar lifetimes, and

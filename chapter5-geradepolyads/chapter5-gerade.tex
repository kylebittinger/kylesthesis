...
$T_{1,2}$ rovibrational levels by generalizing from a two-level
system.  
...
\begin{equation}
  ...
  \end{split}
\end{equation}
where $\alpha$ is the mixing angle between $\ket{s}$ and $\ket{t}$, $0
\leq \alpha \leq 0.5$.  The fluorescence intensity of the mixed
states, relative to a pure $S_1$ state, is
\begin{equation}
  \begin{split}
    I_1(t) &= \frac{(1 - \alpha^2)^2}{\tau_s} \; \exp 
          \left[
            - \frac{(1 - \alpha^2) t}{\tau_s}
          \right]\\
    I_2(t) &= \frac{\alpha^4}{\tau_s} \; \exp 
          \left[
            - \frac{\alpha^2 t}{\tau_s}
          \right].
  \end{split}
\end{equation}
The ratio of the fluorescence intensities changes with time:
\begin{equation}
  ...
  \exp
  \left[
    - \frac{(1 - 2\alpha^2) t}{\tau_s}
  \right].
\end{equation}  
The initensity ratio at $t=0$ is determined by the prefactor $( (1 -
\alpha^2)/\alpha^2 )^2$.  In the limit of long time delay, $I_{12}(t)$
...
fraction, $\alpha^2$.  At small values of the mixing fraction, $0.001
\leq \alpha^2 \leq 0.2$, the intensity ratio changes rapidly at a time
delay between $5\tau_s$ and $15\tau_s$.  When the mixing fraction
approaches its limiting value of 0.5, the prefactor $( (1 -
\alpha^2)/\alpha^2 )^2$ causes the intensities $I_1$ and $I_2$ to be
...
respectively.  The expression has a simple form when written in terms

...
Ejection by Laser-Excited Metastables \cite{sneh89, sneh91,
  humphrey97} (SEELEM) and LIF spectroscopy are complementary tools
that have been used to gain insight into singlet $\sim$ triplet
interactions in the $\tilde{A} ^1A_u \leftarrow \tilde{X}
^1\Sigma_g^+$ ($S_1$-$S_0$) electronic spectrum of acetylene,
\ce{C2H2} \cite{humphrey97, mishra04, altunata00, altunata02}.  LIF
detection is limited to short-lived ($\tau_{radiative}<10$ $\mu$s),
strongly fluorescing singlet states, while SEELEM detection is
sensitive only to long-lived ($\tau>300$ $\mu$s) states with vertical
electronic excitation above a threshold energy set by the work
function of the metal used as the SEELEM detector surface. Therefore,
SEELEM and LIF detection channels observe mutually exclusive sets of
eigenstates that arise from spin-orbit mixed $S_1$, $T_{3,2,1}$
zero-order basis states.  A comparison of simultaneously recorded
...
The information gained from comparison of acetylene SEELEM / LIF
spectra can yield a mechanistic description of singlet $\sim$ triplet
interaction, and holds promise for describing the structure and
dynamics of other small polyatomic species \cite{jung07, allen07}.
...
$S_1 \leftarrow S_0$ transition.  Spectroscopic patterns in SEELEM
arising from the effects of the local $T_3$ perturber in have been
discussed by several authors \cite{humphrey97, altunata00,
  altunata-thesis, cunningham-thesis, thom-thesis, mishra04}.
...
manifold of dark $T_{1,2}$ states \cite{dupre91, dupre95a}.  The $3
\nu_3$ level in particular has been heavily studied due to a local
$S_1 \sim T_3$ level crossing at low $J$.

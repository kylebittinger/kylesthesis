...
\end{equation}
For the Q(3) transition of $3^34^1$ \Ka{0}, the state appearing at
...
\Ka{0} sublevels are on the order of 0.1 \rcm.  The matrix element for
the local $T_3$ doorway level ($\\Delta E_{s\ell} < 0.3$ \rcm)
observed in $S_1$ $3^3$ \Ka{1} was determined to be approximately 0.1
\rcm\ \cite{mishra04}.  However, no local doorway state is observed in
the $3^36^1$ \Ka{0} and $3^34^1$ \Ka{0} sublevels, so $\Delta
E_{s\ell} > 2$ \rcm.  This indicates that the $S_1 \sim T_3$ matrix
elements for the $3^36^1$ \Ka{0} and $3^34^1$ \Ka{0} sublevels must be
far larger than that observed for the $3^3$ \Ka{1} sublevel.
...
fractional bright state character.  In Chapter 2, we showed that if
the doorway state is remote ($\Delta E_{s\ell} > 2$ \rcm), the
...
percent.  On the outer edge of the SEELEM intensity distribution, we
assume that the fractional $S_1$ character of the SEELEM-detectable
eigenstates approaches zero.  In this limit, the SEELEM detection
probability is dominated by the polynomial factor, and is proportional
to the fractional bright state character squared.  Surprisingly, the
causes a \emph{narrowing} effect on the outer edge of the SEELEM
intensity distribution.  The full width of a squared Lorentzian
distribution is approximately 0.64 times the full width of a standard
Lorentzian.  

\NOTE{I am including the prev. paragraph for now, although the
  argument here is a bit tenuous.  The surprising result here is that
  the SEELEM envelope appears so narrow in our measurements.  I am
  arguing, in effect, that the intensities for the majority of the
  eigenstates in our SEELEM spectrum have a quasi-polynomial
  dependence on bright state character.  This means that, among the
  ensemble of SEELEM-active states, the survival probability factor is
  constant (squeezed Lorentzian) or varies linearly (true Lorentzian)
  with bright state character.  Such an argument may seem to conflict
  with Kevin Cunningham's measurements of SEELEM lifetimes as
  relatively uniform and universally less than 280 \microsec.
  However, an upper limit on fluorescence lifetime of 280 \microsec\
  means that the survival probability is approximately constant for
  SEELEM-detectable states.  The quasi-polynomial relationship between
  SEELEM detection probability and fractional bright state character
  follows.}
...
involving torsional motion, $3^34^1$ \Ka{0}, is found to have an
...
$3^36^1$ \Ka{0}.  This result is in contrast to other experimental
...
in $3^34^1$ \Ka{0}?  This is a surprising result.  One possible
explanation is that $3^34^1$ \Ka{0} gains $T_3$ character through
$b$-axis Coriolis coupling with the \Ka{1} sublevel of $3^36^1$
\Ka{1}.  The $3^36^1$ \Ka{1} is known to be strongly perturbed by a
level of $T_3$.  The matrix element for $b$-axis Coriolis interaction
between $3^34^1$ \Ka{0} and $3^36^1$ \Ka{1} is approximately 1.4 \rcm.
The energy separation between the $3^34^1$ \Ka{0} level and the
$3^36^1$ \Ka{1} sublevel is approximately 42 \rcm.  This leads to a
mixing angle of about 0.03.  The observed splitting in $3^36^1$ \Ka{1}

...
constants for the vibrational level.  The $T_3$ electronic state is
best described in a Hund's case ($b$) basis, based on the magnitude of
the carbon atomic spin-orbit matrix element \cite{field04}.  For
$T_3$, the model of a near-symmetric prolate top is still appropriate
in the non-planar \emph{trans}-bent equilibrium geometry predicted by
\emph{ab initio} calculations \cite{ventura03, thom07}, due to the
small mass of the hydrogen atoms relative to carbon.  The energy
...
% $K$=1 and 2 sublevels of a $T_3$ vibration having $a_g$ symmetry.
% Triplet vibrational sublevels with $K \ge 2$ have the same structure
% as $K=1$, except for the absence of any rotational levels with
% $N<K$.
Since $S=0$ for the $S_1$ electronic state, only one component $N=J$
is present for all $S_1$ rotational levels.
...
rovibrational states of $S_1$ and $T_3$ is given by the product of
...
\ref{eq:components}. \TODO{Finish para.}

When $H_{ST}^2$ is on the order of $2B_T$, mixing between the $S_1$
and $T_3$ levels occurs at several values of $J$; these are the
energetically distant $T_3$ doorway levels with which we are most
concerned. \TODO{Finish para.  Lead in to next section.}
...
interacts.  We define an energetically distant doorway level as one
which is more than 2 \rcm\ away from the $S_1$ level in rotationless
energy.  In this section, we derive the dependence of the
center-of-gravity on time delay for an incoherent ensemble of mixed
...
SEELEM center-of-gravity is biased in the direction of the doorway
level relative to the LIF center of gravity, according to the formula
\begin{equation}
  \Delta E_{\text{ave}} \propto \alpha_{S_1T_3}
  \left (
    \frac{\braket{H_{T_3T_{1,2}}}}{\Delta E_{S_1T_3}}
  \right ).
\end{equation}
We extend the idea of eigenstate intensity distributions to
fluorescence measurements by considering intensities in the
\emph{incoherent, frequency domain} LIF spectrum within a time window
$t$ to $t+dt$ after excitation of the molecule.
...
We examine the relative fluorescence intensity among an incoherent
ensemble of singlet$\sim$triplet mixed eigenstates after a time delay
...
The value of $S_1$ character with the maximum relative
intensity is found by taking the derivative of eigenstate intensity
(Equation \ref{eq:int-m}) with respect to fractional $S_1$
character, $a_m^2$.  Doing this, we find
\begin{equation}
...
\begin{equation}
  ...
  P_{\text{SEELEM}} \propto 
    ...
    \right ].
\end{equation}
This is a good approximation to the overall detection probability of
...
energetically distant (see Chapter 2).  The SEELEM detection
probability equation above has the same functional form as the delayed
LIF intensity, Equation \ref{eq:int-m}.  The detection probability is
at a maximum for eigenstates with 0.17\% fractional $S_1$ character.
In this approximation, the SEELEM detector may be viewed as an extreme
...
We make the assumption of a doorway-coupled model Hamiltonian, in
which all matrix elements between the $S_1$ level and the local
ensemble of $T_{1,2}$ rovibrational levels are zero.  The Hamiltonian
contains nonzero matrix elements between the $S_1$ level and an
energetically distant $T_3$ level, and also between the $T_3$ level
and the local ensemble of $T_{1,2}$ levels.  In such a model
Hamiltonian, no interference pathways exist between the $S_1$ level
and the local manifold of $T_{1,2}$ levels.  By applying successive
matrix rotations to the doorway-coupled model Hamiltonian, we can
construct a direct-coupled model Hamiltonian (see Chapter 3).  This
modified Hamiltonian has nonzero matrix elements between the $S_1$
level and all $T_{1,2}$ levels.  These matrix elements
\emph{implicitly} include the effects of the doorway level in the
modified, direct-coupled Hamiltonian.  Our discussion below focuses on
the local energy block of such a modified model Hamiltonian.

Consider a Hamiltonian containing one $S_1$ level $\ket{s}$ and one
dark $T_{1,2}$ level $\ket{t}$.  Let us define the two mixed states
$\ket{1}$ and $\ket{2}$ as
\begin{equation}
  \begin{split}
...
The normalized fluorescence intensities of the mixed states are
\begin{equation}
  \begin{split}
...
small values of the mixing fraction, $0.001 \leq a^2 \leq 0.2$,
the center of gravity changes rapidly from $E_1$ to $E_2$ at a time
delay between $5\tau_s$ and $15\tau_s$.  When the mixing fraction is
nearly 0.5, the initial center of gravity at $t=0$ is midway between
$E_1$ and $E_2$, because $\ket{1}$ and $\ket{2}$ have similar
intensities at $t=0$.  In this case, the center of gravity changes
slowly as a function of time.  The total magnitude of change in center
of gravity is also decreased, due to the decreased energy separation
between the centers of gravity at $t=0$ and $t \rightarrow \infty$.

\begin{figure}
  \caption{Time development of the center of gravity for a two state
    system.}
  \label{fig:cog-devel}
  \centering
...
Having established the general properties for the time-dependence of
the center of gravity, we expand our model to include many
rovibrational levels of $T_{1,2}$.  The nominal $S_1$ eigenstate,
$\ket{1}$, is defined to have a fractional $S_1$ character of
$(1-a^2)$, as before.  The remaining fractional $S_1$ character,
$a^2$, is distributed among a set of $N$ nominal $T_{1,2}$ eigenstates
$\lbrace \ket{2} ... \ket{N+1}\rbrace$.  The set of nominal $T_{1,2}$
eigenstates has an average fractional $S_1$ character of
\begin{equation}
  \braket{a_m^2} = \frac{a^2}{N},
\end{equation}
and a resultant time-dependent fluorescence intensity of
\begin{equation}
  \label{eq:int-N}
  I_m(t) = \frac{a_m^4}{N^2 \tau_s} \;
           \exp \left[
             -\frac{a_m^2 \, t}{N \tau_s} 
           \right].
\end{equation}
The intensity-weighted center of gravity for the set of $N$ nominal
$T_{1,2}$ eigenstates is determined by the energy distribution of
fractional $S_1$ character,
\begin{equation}
  E_{m} = \sum_{m=2}^{N+1} E_m \times a_m^2.
\end{equation}

The energy distribution of fractional $S_1$ character is influenced by
the matrix element and relative energy for the energetically distant
doorway level.  As a result, the center of gravity $E_{t}$ is
biased, relative to $E_1$, in the direction of the $T_3$ doorway
level.  The magnitude of this energy offset is given by the product of
the slope of average $S_1$ character with energy,
\begin{equation}
  \frac{\partial \braket{a_m^2}}{\partial E} 
  \propto 
  \frac{1}{\Delta E_{S_1T_3}},
\end{equation}
and the local $S_1 \sim T_{1,2}$ coupling width, 
\begin{equation}
  \sigma^2 \propto
  \left(
    \frac{ H_{S_1T_3} \braket{H_{T_3T_{1,2}}} }
         { \Delta E_{S_1T_3} }
  \right)^2,
\end{equation}
where $H_{S_1T_3}$ is the matrix element between the $S_1$ level and
the doorway state, $\braket{H_{T_3T_{1,2}}}$ is the average $T_3 \sim
T_{1,2}$ matrix element, and $\Delta E_{S_1T_3}$ is the energy
difference between the $S_1$ level and the doorway state.  A
derivation is given in Chapter 2.

\TODO{Actually include the derivation explicitly in Chapter 2.
  Implicitly, the two terms of the above formula follow from these two
  results: 
  \begin{enumerate}
  \item The energy dependence of fractional bright state character is
    given by the cross term/square term ratio from the mixed basis
    formaulation.  The ratio depends only on the $S_1 \sim T_3$ energy
    separation (Chapter 2, formula 1.5), thus its slope must depend
    only on energy separation.  The slope must decrease with
    increasing energy separation, so I surmised an inverse
    proportionality.
  \item Chapter 3, formula 1.13 gives the spectral variance for a
    direct-coupled Hamiltonian as the sum of the matrix elements
    squared.  The magnitude of the local matrix elements in the
    embedded bright state formulation is given by the product of the
    $S_1 \sim T_3$ mixing fraction and the average $T_3 \sim T_{12}$
    matrix element (Chapter 2, formula 1.7).
  \end{enumerate}
}

% The ratio of total intensity between the nominal $S_1$ eigenstate,
% $\ket{1}$, and the set of nominal $T_{1,2}$ eigenstates is
% \begin{equation}
%   I_{1m}(t) = \frac{I_1(t)}{I_m(t)} = 
%   \left(
%     N \frac{(1 - a^2)}{a^2}
%   \right)^2
%   \exp
%   \left[
%     - \frac{(1 - 2a^2/N) t}{\tau_s}
%   \right].
% \end{equation}

To examine the effect of increasing the effective number of states in
the model system, the intensity-weighted center-of-gravity is plotted
as a function of time delay in Figure \ref{fig:cog-devel-many}.  A
value of $a_m^2=0.25$ is used for the figure, and four values of $N$
are examined: $N=1,10,20,30$.  As the number of nominal $T_{1,2}$
states is increased, the initial ($t=0$) center of gravity for the
entire system moves closer to $E_1$.  The center of gravity then
changes more rapidly from $E_1$ to $E_m$, the $T_{1,2}$ ensemble
center of gravity.  The effect of increasing the number of local
$T_{1,2}$ states in the system is to increase the effective lifetime
of the nominal triplet eigenstates.  This, in turn, increases the
contrast between the intensity of the nominal $S_1$ eigenstate and the
rest of the ensemble, bringing about a more rapid shift in the center
of gravity with time.

\begin{figure}
  \caption{Time development of the center of gravity for a system
    containing $N$ levels of $T_{1,2}$ basis character.}
  \label{fig:cog-devel-many}
  \centering
  \includegraphics[width=6in]{cog-development-many.pdf}
\end{figure}
...
mixture of mode 4 and mode 6 basis state character \cite{merer08,
  virgo07}.
